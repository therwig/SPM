...
histopath   = "/mnt/t3nfs01/data01/shome/cheidegg/d/SPM/tmp/bundle/170307145157801993/histo" 
smearpath   = "/mnt/t3nfs01/data01/shome/cheidegg/d/SPM/tmp/bundle/170307145157801993/smear" 
plotpath    = "/mnt/t3nfs01/data01/shome/cheidegg/d/SPM/tmp/bundle/170307145157801993/plot" 
spmpath      = "/mnt/t3nfs01/data01/shome/cheidegg/d/SPM"
exts        = "C,png,pdf,root"   
preliminary = "Preliminary"
lumis       = [ '35p9fb','35p9fb','35p9fb','35p9fb','35p9fb' ]
energy      = "13"
...
class Model():
    def __init__(self):
        self.name       = "TChiZZ4L"
        self.xsecpath   = spmpath+"/xsec/xsec_NeuNeu.txt"
        self.brcorr       = eval("1.0/1000")
        self.mode       = "braz"
        self.plane      = "obs_xs"
        self.deltaM     = float("1") if "1" else 1
        self.isNloNll   = (""!="True")
        self.posY       = [float(f) for f in "150,100,70,55".split(",")] if "150,100,70,55" else []
        self.binningX   = "41,100,1025"
        self.binningY   = "1,0.5,1.5"
        self.rangeX     = [float(f) for f in "100,1000".split(",")]
        self.rangeY     = [float(f) for f in "0.05,500".split(",")]
        self.rangeZ     = [float(f) for f in "0.001,2".split(",")]
        self.legendX    = "Higgsino mass m_{#tilde{#chi}_{1}^{0}}"
        self.legendY    = ""
        self.nDivX      = int("505")
        self.nDivY      = int("505")
        self.logX       = True if "[MODEL:logX]"=="True" else False
        self.logY       = True if "True"=="True" else False
        self.text       = ['pp #rightarrow #tilde{#chi}_{1}^{0}#tilde{#chi}_{2}^{0} #rightarrow ZZ#tilde{G}#tilde{G}','BR(#tilde{#chi}_{1}^{0}#rightarrow Z#tilde{G})=1, m_{#tilde{#chi}_{2}^{0}}=m_{#tilde{#chi}_{1}^{0}}=m_{#tilde{#chi}_{1}^{#pm}}, m_{#tilde{G}} = 1 GeV']
        self.diag       = []
        self.smoothCont = ("False"=="True")
    def setGraphs(self):
        add = ""
        if self.smoothCont: add = "_smoothed"
        self.gr_obs     = ""  if ""  else "gr_obs" +add
        self.gr_op1s    = "" if "" else "gr_op1s"+add
        self.gr_om1s    = "" if "" else "gr_om1s"+add
        self.gr_exp     = ""  if ""  else "gr_exp" +add
        self.gr_ep1s    = "" if "" else "gr_ep1s"+add
        self.gr_em1s    = "" if "" else "gr_em1s"+add
...
class ThePlot():
    ...
    def drawText(self):
        ...
        if nText+nLeg > 0:
            ...
            graphWhite.SetPoint(2, self.xmax, self.ymax/(10*(nText+nLeg-1)))
            graphWhite.SetPoint(3, self.xmin, self.ymax/(10*(nText+nLeg-1)))
        ...
        text      = []
        textSizes = []
        nominal   = 0.143446295867 # 0.188273263326
        ...
        if self.model.isNloNll:
            textNLL = ROOT.TLatex(0.63, 0.89-self.textCorr-0.02, "NLO-NLL excl.")
            #textNLL = ROOT.TLatex(0.63, 0.905-0.07*nText-0.02, "NLO-NLL excl.")
            #if self.c.GetLogy(): textNLL = ROOT.TLatex(self.xmin+66*xRange/100, 340, "NLO-NLL excl.")
            #else               : textNLL = ROOT.TLatex(self.xmin+66*xRange/100, self.ymax-(0.35+0.75*nText)*yRange/100*10, "NLO-NLL excl.")
            textNLL.SetNDC()
...
class BrazPlot():
    ...
    def drawLegend(self):
        ...
        #LObsP.SetPoint(0, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)+0.02-0.04))
        #LObsP.SetPoint(1, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)+0.02-0.04))
        #LObsP.SetPoint(0, self.parent.xmin+3 *xRange/100, self.parent.ymax-(0.35+0.75*nText)*yRange/100*10)
        #LObsP.SetPoint(1, self.parent.xmin+10*xRange/100, self.parent.ymax-(0.35+0.75*nText)*yRange/100*10)
        LObs = ROOT.TGraph(2)
        LObs.SetName("LObs")
        ...
        #LObs.SetPoint(0, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)+0.01-0.04))
        #LObs.SetPoint(1, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)+0.01-0.04))
        #LObs.SetPoint(0, self.parent.xmin+3 *xRange/100, self.parent.ymax-(0.50+0.75*nText)*yRange/100*10)
        #LObs.SetPoint(1, self.parent.xmin+10*xRange/100, self.parent.ymax-(0.50+0.75*nText)*yRange/100*10)
        LObsM = ROOT.TGraph(2)
        LObsM.SetName("LObsM")
        ...
        #LObsM.SetPoint(0, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)-0.04))
        #LObsM.SetPoint(1, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)-0.04))
        #textObs = ROOT.TLatex(0.23, 0.905-0.07*(nText)-0.04, "Observed #pm 1 #sigma_{theory}") # -0.02 cause text is too large
        ##LObsM.SetPoint(0, self.parent.xmin+3 *xRange/100, self.parent.ymax-(0.65+0.75*nText)*yRange/100*10)
        ##LObsM.SetPoint(1, self.parent.xmin+10*xRange/100, self.parent.ymax-(0.65+0.75*nText)*yRange/100*10)
        ##textObs = ROOT.TLatex(self.parent.xmin+11*xRange/100, self.parent.ymax-(0.65+0.75*nText)*yRange/100*10, "Observed #pm 1 #sigma_{theory}")
        textObs.SetNDC()
        ...
        #LExp.SetPoint(0, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)+0.015-0.06-0.04))
        #LExp.SetPoint(1, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)+0.015-0.06-0.04))
        #LExp.SetPoint(0, self.parent.xmin+3 *xRange/100, self.parent.ymax-(1.15+0.75*nText)*yRange/100*10)
        #LExp.SetPoint(1, self.parent.xmin+10*xRange/100, self.parent.ymax-(1.15+0.75*nText)*yRange/100*10)
        ...
        #LExp1S.SetPoint(0, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)+0.025-0.06-0.04))
        #LExp1S.SetPoint(1, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)+0.025-0.06-0.04))
        #LExp1S.SetPoint(2, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)+0.005-0.06-0.04))
        #LExp1S.SetPoint(3, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)+0.005-0.06-0.04))
        ...
        #LExp2S.SetPoint(0, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)+0.03-0.06-0.04))
        #LExp2S.SetPoint(1, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)+0.03-0.06-0.04))
        #LExp2S.SetPoint(2, NDCtoUser(0.22, True), NDCtoUser(0.905-0.07*(nText)     -0.06-0.04))
        #LExp2S.SetPoint(3, NDCtoUser(0.16, True), NDCtoUser(0.905-0.07*(nText)     -0.06-0.04))
        #textExp = ROOT.TLatex(0.23, 0.905-0.07*(nText)-0.06-0.04, "Expected #pm 1(2) #sigma_{experiment}")
